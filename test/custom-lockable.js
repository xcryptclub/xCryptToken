const Contract = artifacts.require("./xCryptToken.sol");
...
contract("Custom Lockable", function(accounts) {
  describe("Canlock Ruleset", () => {
    ...
    it("must not allow non-admins/non-owner to disable locking.", async () => {
      await customLockable.disableLocking({ from: accounts[1] }).should.be.rejectedWith(EVMRevert);

      await customLockable.disableLocking({ from: accounts[2] }).should.be.rejectedWith(EVMRevert);
    });
    ...
    it("must allow the owner to completely disable locking feature and only once.", async () => {
      ...
      await customLockable.disableLocking({ from: owner }).should.be.rejectedWith(EVMRevert);
    });

    it("must allow an admin to completely disable locking feature and only once.", async () => {
      await customLockable.addManyAdmins([accounts[1], accounts[2], accounts[3]]);
  ...
  describe("When Canlock Is Disabled Ruleset", () => {
    ...
    it("must not allow adding locks.", async () => {
      await customLockable.addLock(accounts[9], releaseDate, { from: owner }).should.be.rejectedWith(EVMRevert);

      await customLockable.addLock(accounts[9], 0, { from: owner }).should.be.rejectedWith(EVMRevert);
    });
  ...
  describe("Add Lock Ruleset", () => {
    ...
    it("must not allow zero address to be locked.", async () => {
      await customLockable.addLock(ZERO_ADDRESS, releaseDate, { from: owner }).should.be.rejectedWith(EVMRevert);

      await customLockable.addManyLocks([accounts[1], accounts[2], ZERO_ADDRESS], releaseDate);
    ...
    it("must not allow owner to be locked.", async () => {
      await customLockable.addLock(owner, releaseDate, { from: owner }).should.be.rejectedWith(EVMRevert);

      await customLockable.addManyLocks([accounts[1], accounts[2], owner], releaseDate);
    ...
    it("must not allow admins to be locked.", async () => {
      ...
      await customLockable.addLock(accounts[1], releaseDate, { from: owner }).should.be.rejectedWith(EVMRevert);

      await customLockable.addLock(accounts[2], releaseDate, { from: owner }).should.be.rejectedWith(EVMRevert);
      await customLockable.addLock(accounts[3], releaseDate, { from: owner }).should.be.rejectedWith(EVMRevert);

      await customLockable.addManyLocks([accounts[1], accounts[2], accounts[3], accounts[4]], releaseDate);
    ...
    it("must not allow unlocking via addManyLocks function", async () => {
      await customLockable.addManyLocks([accounts[1], accounts[2], accounts[3], accounts[4]], 0).should.be.rejectedWith(EVMRevert);
    });
  ...
  describe("ERC20 Lock Ruleset", () => {
    ...
    beforeEach(async () => {
      ...
      (await customLockable.balanceOf(accounts[1])).should.bignumber.equal(ether(100));

      (await customLockable.balanceOf(accounts[2])).should.bignumber.equal(ether(100));
    });
    ...
    it("must not allow allow transfer, transferFrom, approve, increaseApproval, decreaseApproval if locked", async () => {
      ...
      await customLockable.transfer(owner, ether(50), { from: accounts[1] }).should.be.rejectedWith(EVMRevert);
      await customLockable.approve(accounts[5], ether(50), { from: accounts[1] }).should.be.rejectedWith(EVMRevert);
      await customLockable.increaseApproval(accounts[5], ether(5), { from: accounts[1] }).should.be.rejectedWith(EVMRevert);
      await customLockable.decreaseApproval(accounts[5], ether(1), { from: accounts[1] }).should.be.rejectedWith(EVMRevert);

      //Account 2 is not locked
      await customLockable.transfer(accounts[5], ether(20), { from: accounts[2] });
      await customLockable.approve(accounts[5], ether(20), { from: accounts[2] });
      await customLockable.increaseApproval(accounts[5], ether(2), { from: accounts[2] });
      await customLockable.decreaseApproval(accounts[5], ether(1), { from: accounts[2] });
      await customLockable.approve(accounts[6], ether(50), { from: accounts[5] }).should.be.rejectedWith(EVMRevert);
      await customLockable.transferFrom(accounts[2], accounts[7], ether(2), { from: accounts[5] });
      await customLockable.approve(accounts[2], ether(10), { from: accounts[5] }).should.be.rejectedWith(EVMRevert);

      //Account 5 is locked
      await customLockable.transferFrom(accounts[5], accounts[7], ether(2), { from: accounts[2] }).should.be.rejectedWith(EVMRevert);

      (await customLockable.balanceOf(accounts[5])).should.bignumber.equal(ether(20));
      (await customLockable.balanceOf(accounts[2])).should.bignumber.equal(ether(78));

      await customLockable.transfer(accounts[1], ether(50), { from: owner });
      (await customLockable.balanceOf(accounts[1])).should.bignumber.equal(ether(150));
    });
    ...
    it("must allow allow transfer, transferFrom, approve, increaseApproval, decreaseApproval after release date", async () => {
      ...
      await customLockable.approve(accounts[5], ether(50), { from: accounts[1] });
      await customLockable.increaseApproval(accounts[5], ether(5), { from: accounts[1] });
      await customLockable.decreaseApproval(accounts[5], ether(1), { from: accounts[1] });
      ...
      await customLockable.increaseApproval(accounts[5], ether(2), { from: accounts[2] });
      await customLockable.decreaseApproval(accounts[5], ether(1), { from: accounts[2] });
      await customLockable.approve(accounts[6], ether(50), { from: accounts[5] });
      await customLockable.transferFrom(accounts[2], accounts[7], ether(2), { from: accounts[5] });
      await customLockable.approve(accounts[2], ether(10), { from: accounts[5] });

      //Account 5 is locked
      await customLockable.transferFrom(accounts[5], accounts[7], ether(2), { from: accounts[2] });
